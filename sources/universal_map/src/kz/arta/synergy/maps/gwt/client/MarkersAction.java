...
//                map.setZoom(16);
        ...
        marker.addRightClickListener(new Marker.RightClickHandler() {
            ...
            public void handle(MouseEvent event) {
                ...
                SC.ask("Вы действительно хотите удалить привязку объекта?", new BooleanCallback() {
                    @Override
...
//                }
    ...
    private void codeLatLng(final double lat, final double lng, final Marker marker) {
        SC.ask("Вы действительно хотите изменить положение объекта?", new BooleanCallback() {
            @Override
            public void execute(Boolean aBoolean) {
                if (aBoolean) {
