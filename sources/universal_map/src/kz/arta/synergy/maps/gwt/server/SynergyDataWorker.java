...
import kz.arta.synergy.maps.gwt.client.Filter;
...
//            System.out.println("buffer = "+buffer.toString());
            ...
            if(genreJsonObject.containsKey("uuid")&&genreJsonObject.containsKey("data")){
                ...
                if(genreJsonObject.containsKey("data")){
                    JSONArray genreArray = (JSONArray) genreJsonObject.get("data");
                    HashMap fieldMap = so.getMapField();
                    for(int i=0;i<genreArray.size();i++){
...
//                            System.out.println("TypeValue = "+so.getTypeValue());
                        ...
                        String id = ((JSONObject)genreArray.get(i)).get("id").toString();
                        Object v = ((JSONObject) genreArray.get(i)).get("key");
                        String value=null;
                        if(v!=null)
                        value =  v.toString();
                        else {
                            v = ((JSONObject) genreArray.get(i)).get("value");
...
//            url = new URL(Properties.getSynergyServerUrl()+Properties.API_LIST_URL + Properties.FORM_UUID);
            url = new URL(Properties.getSynergyServerUrl()+Properties.API_SEARCH_URL+"?"+getUserFederationList(sso_hash)+"showDeleted=false&getDocIds=true");
            ...
            for(int i=0;i<genreJsonObject.size();i++){
                ...
                String soDataUUID = ((JSONObject)genreJsonObject.get(i)).get("dataUUID").toString();
                String soDocumentId = ((JSONObject)genreJsonObject.get(i)).get("documentID").toString();
                SportObject so = getSportObject(soDataUUID);
                if(so!=null&&so.getVersion()>=Properties.LOWER_VERSION&&so.getFormVersion()>=Properties.LOWER_FORM_VERSION){
                    so.setDocumentId(soDocumentId);
                    sportObjects.add(so);
                }
            }
...
//                    System.out.println(((JSONObject)genreArray.get(i)).get("name").toString());
                    substr="formUUID="+Properties.FORM_UUID+"&search="+ java.net.URLEncoder.encode(((JSONObject)genreArray.get(i)).get("name").toString())+"&field="+Properties.API_SECURITY_FIELD+"&type=exact&";

                }else{
                    substr="formUUID"+i+"="+Properties.FORM_UUID+"&search"+i+"="+java.net.URLEncoder.encode(((JSONObject)genreArray.get(i)).get("name").toString())+"&field"+i+"="+Properties.API_SECURITY_FIELD+"&type"+i+"=exact&";
                }
//                System.out.println(((JSONObject)genreArray.get(i)).get("name").toString());
                groups+=substr;
            }
    ...
    private List<DictionaryItem> loadDictionary(String dictionaryKey) throws SynergyDataException{
        ...
        try {
            ...
            JSONArray genreArray = (JSONArray) genreJsonObject.get("data");
            for (int i = 0; i < genreArray.size(); i++) {
                if(((JSONObject)genreArray.get(i)).get("id").equals(dictionaryKey)) {
//                    System.out.println(((JSONObject)genreArray.get(i)).get("values"));
                    if(((JSONObject)genreArray.get(i)).get("values")!=null){
                        JSONArray items = (JSONArray)((JSONObject)genreArray.get(i)).get("values");
                        for(int j=0;j<items.size();j++){
    ...
    public List<Filter> getFilters() throws SynergyDataException {
        FilterConfig[] filtersConfig = Configuration.FILTERS;
        List<Filter> filters = new ArrayList<Filter>();
        for(int i=0;i<filtersConfig.length;i++){
            FilterConfig filterConfig = filtersConfig[i];
            Filter filter = new Filter();
            filter.setCaption(filterConfig.getFilterText());
            filter.setFieldId(filterConfig.getFilterFieldKey());
            if(filterConfig.getFilterDictionaryKey()!=null&&filterConfig.getFilterDictionaryKey().length()>0){
                filter.setDictionary(loadDictionary(filterConfig.getFilterDictionaryKey()));
            }
            filters.add(filter);
        }
        return filters;
